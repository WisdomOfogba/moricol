...
export default function CourseCategories({
  ...
  const categoriesList = categories.map((c, i) => (
    <li
      key={i}
      className={`w-[160px] border-b pb-1 text-center md:w-full ${activeCategory === i ? "border-primary-500 text-primary-500" : "border-gray-700 text-gray-700"}`}
    >
  ...
  return (
    <section className="w-full">
      ...
      <div className="w-full overflow-auto no-scrollbar">
        <ul className="mb-8 flex w-max md:w-full justify-between">
          {categoriesList}
        </ul>
      </div>

      <div className="grid grid-cols-1 gap-x-6 gap-y-7 bg-[#F5F7FA] px-4 py-8 sm:grid-cols-2 md:px-14 lg:grid-cols-4 xl:grid-cols-5">
